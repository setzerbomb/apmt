function MiningT(root)
  ...
  function self.up()
    return move(turtle.up,self.attackUp,self.digUp,function() objects.position.addY(); self.saveAll(); end)
  end

  function self.down()
    return move(turtle.down,self.attackDown,self.digDown,function() objects.position.subY(); self.saveAll(); end)
  end
